...
function kataToHira(s) {
  ...
  for (const ch of s) {
    ...
    if (code >= 12449 && code <= 12534) out += String.fromCodePoint(code - 96);
    else out += ch;
  }
  return out;
}
...
function kanaToKo(input) {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    const ch = s[i];
    if (ch === "\u3063") {
      const r2 = addJong(out, JONG_S);
      if (r2) out = r2;
      continue;
    }
    if (ch === "\u3093") {
      const r2 = addJong(out, JONG_N);
      out = r2 ?? out + "\u3134";
      continue;
    }
    if (ch === "\u30FC") continue;
...
function splitFurigana(surface, reading) {
  let pre = 0;
  while (pre < surface.length && pre < reading.length && surface[pre] === reading[pre] && isKanaCh(surface[pre]))
    pre++;
  let s = surface.length;
  let r2 = reading.length;
  while (s > pre && r2 > pre && surface[s - 1] === reading[r2 - 1] && isKanaCh(surface[s - 1])) {
    s--;
    r2--;
  }
  return {
    ...
    rt: reading.slice(pre, r2),
    after: surface.slice(s)
  };
}
var isVocal = (l) => l.jp.length > 0 || !!l.pron || !!l.mean;
function activeIndexAt(lines, now, _duration) {
  let idx = -1;
...
function withTimings(lines, tArr) {
...
var DEFAULT_LABELS = {
  ...
  full: "\uC804\uCCB4 \uBCF4\uAE30",
  line: "\uC904 \uB2E8\uC704",
  guide: "\uB2E8\uC5B4\uC5D0 \uB9C8\uC6B0\uC2A4\uB97C \uC62C\uB9AC\uAC70\uB098(\uBAA8\uBC14\uC77C\uC740 \uD0ED) \uB204\uB974\uBA74 \uC77D\uB294 \uBC95\uACFC \uB73B\uC774 \uB098\uD0C0\uB0A9\uB2C8\uB2E4. \uBC1C\uC74C\xB7\uB73B\uC740 \uC704 \uBC84\uD2BC\uC73C\uB85C \uCF1C\uACE0 \uB04C \uC218 \uC788\uC5B4\uC694.",
  ...
  return /* @__PURE__ */ jsx(
    "button",
    {
      ...
      className: cn(
        ...
        on ? "border-primary/40 bg-accent text-accent-foreground" : "border-border text-muted-foreground line-through opacity-60"
      ),
      children
    }
  );
...
function Word({
  ...
  if (!reading && !tok.m) return /* @__PURE__ */ jsx("span", { children: content });
  const aria = tok.t + (reading ? ` (${reading})` : "") + (tok.m ? ` \u2014 ${tok.m}` : "");
  return /* @__PURE__ */ jsx(
    "span",
    {
      role: "button",
...
function LyricPlayer({
  ...
  const [showFurigana, setShowFurigana] = useState2(true);
  const [showPron, setShowPron] = useState2(true);
  const [showMean, setShowMean] = useState2(true);
  const [tip, setTip] = useState2(null);
  const [mode, setMode] = useState2("full");
  const [activeLine, setActiveLine] = useState2(-1);
  ...
  const renderLine = (line, i, opts) => {
    const isSpacer = line.jp.length === 0 && !line.pron && !line.mean;
    if (isSpacer) return /* @__PURE__ */ jsx("div", { "aria-hidden": true, className: "h-3" }, i);
          ...
          children: [
            /* @__PURE__ */ jsx(
              "div",
              {
                ...
                children: line.jp.map((tok, j) => {
                  const reading = showReading(tok.r ?? kanaReading(tok.t));
                  let surface = tok.t;
                  if (showFurigana) {
                    const seg = tok.r && hasKanji(tok.t) ? splitFurigana(tok.t, tok.r) : null;
                    surface = seg && seg.core ? /* @__PURE__ */ jsxs(Fragment, { children: [
                      seg.before,
                      /* @__PURE__ */ jsxs("ruby", { children: [
                        seg.core,
                        /* @__PURE__ */ jsx("rt", { children: seg.rt })
                      ] }),
                      seg.after
                    ] }) : /* @__PURE__ */ jsxs("ruby", { children: [
                      tok.t,
                      /* @__PURE__ */ jsx("rt", { children: "\xA0" })
                    ] });
                  }
                  return /* @__PURE__ */ jsxs("span", { className: "inline-flex flex-col items-center", children: [
                    /* @__PURE__ */ jsx(
                      "span",
                      {
                        className: cn(
            ...
            showMean && line.mean && /* @__PURE__ */ jsx(
              "p",
              {
                className: cn(
        ...
        children: [
          /* @__PURE__ */ jsx(
            "p",
            {
              ...
              children: line.jp.map((tok) => tok.t).join("")
            }
          ),
          lyricLang === "ko" && showPron && line.pron && /* @__PURE__ */ jsx("p", { className: cn("text-accent-2", big ? "text-base" : "text-sm"), children: line.pron })
        ]
      },
      i
    );
  ...
  return /* @__PURE__ */ jsxs("div", { children: [
    /* @__PURE__ */ jsx("div", { className: "mb-6", children: videoId ? /* @__PURE__ */ jsx("div", { className: "aspect-video overflow-hidden rounded-xl border border-border shadow-md [&_iframe]:size-full", children: /* @__PURE__ */ jsx("div", { ref: hostRef, className: "size-full" }) }) : videoFallback ?? /* @__PURE__ */ jsx("div", { className: "flex aspect-video items-center justify-center rounded-xl border border-dashed border-border bg-card/50 text-sm text-muted-foreground", children: "\uC601\uC0C1 \uC900\uBE44 \uC911" }) }),
    meta && /* @__PURE__ */ jsx("div", { className: "mb-6", children: meta }),
    /* @__PURE__ */ jsx("div", { className: "mb-4", children: /* @__PURE__ */ jsxs("div", { className: "inline-flex rounded-full border border-border p-0.5 text-sm", children: [
      /* @__PURE__ */ jsx(
        "button",
        {
          type: "button",
          ...
          className: cn(
            ...
            !sync ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"
          ),
          children: L.full
        }
      ),
    ...
    tip && createPortal(
      /* @__PURE__ */ jsxs(
        "div",
        {
          ...
          children: [
            tip.r && /* @__PURE__ */ jsx("span", { className: "ly-tip-r", children: tip.r }),
            tip.m && /* @__PURE__ */ jsx("span", { className: "ly-tip-m", children: tip.m })
          ]
        }
      ),
      document.body
